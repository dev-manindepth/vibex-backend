...
export class Add {
  ...
  public async backgroundImage(req: Request, res: Response): Promise<void> {
    ...
    const bgImageId: Promise<IUserDocument> = userCache.updateSingleUserPropInCache(
      req.currentUser!.userId,
      'bgImageId',
      publicId
    ) as Promise<IUserDocument>;
    const bgImageVersion: Promise<IUserDocument> = userCache.updateSingleUserPropInCache(
      req.currentUser!.userId,
      'bgImageVersion',
      version
    ) as Promise<IUserDocument>;
    ...
    imageQueue.addImageJob('updateBGImageInDB', { userId: req.currentUser!.userId, imgId: publicId, imgVersion: version.toString() });
    res.status(HTTP_STATUS.OK).json({ message: 'Background Image added' });
  }
